...
impl<'rcu, C: PageTableConfig> CursorMut<'rcu, C> {
    ...
    #[safety::precond::Align(self.0.va, page_size::<C>(C::item_into_raw(item).1))]
    #[safety::precond::Le(self.0.va + page_size::<C>(C::item_into_raw(item).1), self.0.barrier_va.end)]
    #[safety::postcond::KernalMemorySafe]
    #[safety::precond::ValidPAddr(C::item_into_raw(item).0)]
    ...
    ///  - the length is not page-aligned.
    #[safety::postcond::KernalMemorySafe]
    #[safety::postcond::Unaltered(AVAIL1)]
    #[safety::precond::Le(len, self.0.va_barrier.end - self.0.va)]
    #[safety::precond::Align(len, C::BASE_PAGE_SIZE)]
